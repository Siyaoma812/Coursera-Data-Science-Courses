...
# -*- coding: utf-8 -*-
"""
Created on Sat Mar 24 17:41:13 2018
...
def cellular_automaton(input, pattern, generation):
    patternlist = ['...', '..x', '.x.', '.xx', 'x..', 'x.x', 'xx.', 'xxx']
    outputdic = {}
    n = 7
    while n > -1:
        if 2**n < pattern or 2**n == pattern:
            outputdic[patternlist[n]] = 'x'
            pattern = pattern - 2**n
        else:
            outputdic[patternlist[n]] = '.'
        n = n - 1
    i = 0
    previous = input
    while i < generation:
        ...
        while j < len(input)-1:
            for e in patternlist:
                if e == previous[j-1] + previous[j] + previous[j+1]:
                    current = current + outputdic[e]
            j = j + 1
        if j == len(input) - 1:
            for e in patternlist:
                if e == previous[j-1] + previous[j] + previous[0]:
                    current = current + outputdic[e]
        i = i + 1
        previous = current
    return current
